...
class m_preferences extends CI_Model {
    ...
    // get detail client
    function get_detail_client() {
        $sql = "SELECT * FROM client_info LIMIT 0, 1";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
    ...
    /*
      // update client info
      function update_client_info ($params) {
      $sql = "INSERT INTO client_info VALUES (YEAR(CURRENT_DATE), ?, ?, ?, ?, ?, ?, NOW())";
      return $this->db->query($sql, $params);
      }
     * 
     */
    ...
    // update client info
    function update_client_info($params) {
        $sql = "UPDATE client_info SET client_name = ?, client_phone = ?, client_address = ? WHERE client_id = ?";
        return $this->db->query($sql, $params);
    }
